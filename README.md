...
```bash
pnpm install
pnpm test            # 235 tests: 26 contract, 37 sdk, 110 ui, 62 cli
```
...
pnpm test                   # 235 tests, no proof server needed
...
**Testable logic lives outside React.** `round-steps`, `round-actions`,
`round-inputs`, `round-format`, `privacy-view`, `session-storage`,
`provider-security`, `blocked-requests`, `deployment-errors` and `proof-mode` are
pure modules with `node:test` coverage; components stay dumb. `round-actions`
decides what each role may do in the current phase, and why not — so a button can
explain itself before a proof is spent.
`packages/ui/tsconfig.test-build.json` lists exactly what the test build compiles.

**The round is walked, not laid out.** `round-steps` turns it into eleven ordered
steps, each reporting whether it is already done from the public ledger and this
browser's own state, and the screen shows the first that is not. Offering every
action a phase allowed at once let a round leave setup with no reviewer
registered, which cannot be undone — so the order is now structural rather than
advisory.
...
| Passing test suite                         | 235 tests, `pnpm test`; CI `verify` job                                          |
